...
void AxisIndicator::updateImpl() {
  ...
  content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+up_vector*up_len, cylinder_thickness,
                          0.2*up_len, osg::Vec4(1.0f, 1.0f, 0.0f, 1.0f)));

  double x_len = bounding_box.xMax() - bounding_box.xMin();
  content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(0.8*x_len, 0, 0),
                          cylinder_thickness, 0.2*x_len, osg::Vec4(1.0f, 0.0f, 0.0f, 1.0f)));

  double y_len = bounding_box.yMax() - bounding_box.yMin();
  content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(0, 0.8*y_len, 0),
                          cylinder_thickness, 0.2*y_len, osg::Vec4(0.0f, 1.0f, 0.0f, 1.0f)));

  double z_len = bounding_box.zMax() - bounding_box.zMin();
  content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(0, 0, 0.8*z_len),
                          cylinder_thickness, 0.2*z_len, osg::Vec4(0.0f, 0.0f, 1.0f, 1.0f)));
  ...
  std::string corner_string = "("+std::to_string(bounding_box.xMin())+","+std::to_string(
                                bounding_box.yMin())+","+std::to_string(bounding_box.zMin())+")";
